...
const CourseDetail = () => {
  ...
  const { course, purchased } = courseData && courseData;
  if (isError) {
    return <p>Failed to fetch data !Sorry </p>;
  }
      ...
      <div className="bg-[#2D2F31] text-white">
        <div className="max-w-7xl mx-auto py-8 px-4 md:px-8 flex flex-col gap-2">
          ...
          <div className="flex items-center gap-2 text-sm">
            <BadgeInfo size={16} />
            <p>
              Last updated {course?.createdAt?.split("T")[0] || "created at"}
            </p>
          </div>
          <p>
            Students enrolled:{" "}
            {course?.enrolledStudents?.length || "Number of enrolled students"}
          </p>
        </div>
      </div>
        ...
        <div className="w-full lg:w-1/2 space-y-5">
          ...
          <p
            ...
            dangerouslySetInnerHTML={{
              __html: course?.description || " lorem15 ",
            }}
          />
          <Card>
            <CardHeader>
